...
Here is what it looks like in practice. An agent writes `assert True is True` to pass a coverage gate. Technically a valid test. Covers nothing real. slop-mop's Deceptiveness gate blocks the commit and issues a sidequest: write tests that exercise the specific uncovered lines. The agent needs to commit the work to close the task, so it complies. Coverage climbs. Commit goes through. The agent fixed it without me, freeing me up to man another lock.
...
An agent that ships untested code is being overconfident. An agent generating fake tests to pass a quality gate is being deceptive. The Deceptiveness category has an interesting wrinkle: the more dangerous target isn’t the reviewer, but the agent itself — specifically, its tendency to talk itself into thinking the exception is valid and justified. I know what that loop feels like from the inside. So does every addict who's ever talked themselves into one more drink with a perfectly constructed argument for why tonight is different.
...
There's a git flag called `--no-verify`. It tells the entire check system to stand down: god-mode. The flag is all over the training data, so the model wanted to use it. I asked it not to use it. It agreed. It did it again.
...
I know I'm not the only one fighting this. Skim the steering files people write — files like AGENTS.md — to see the all-caps pleas for proof, where each exclamation point is a fingerprint from perceived past gaslightings.
...
At an earlier model’s own suggestion, the wrapper auto-triggers a confession message when it catches a bypass attempt — and it does, eventually. It prints a confession the model wrote about itself after one of its earlier bypasses...
...
I don't know what the fuck is real. I put that in the subtitle and I meant it literally. What I think — not know, think — is that slop-mop mods the scope or bends the slope so that the marbles roll the way I need them to, and that it'll be a boon to you, whatever you're building. Hi, my name is Will. I'm still an unemployed alcoholic who's gonna keep unapologetically using his TI-89 to "cheat" on the test that is life. Keep that in mind while taking any advice from this agent. 
