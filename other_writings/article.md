...
Fair warning up front: this started as a release post and became something closer to an origin story. The personal stuff is load-bearing — it is why the tool works the way it does. If that is not what you came for, slop-mop is here: [https://github.com/ScienceIsNeato/slop-mop/](https://github.com/ScienceIsNeato/slop-mop/). You can head there right now with my blessing.

The short version: slop-mop is a gate system for coding agents. It catches the shortcuts they reach for — fake tests, magic numbers, duplicated blocks, bloated files, dangling imports, commit-hook bypasses — and redirects them into concrete cleanup work instead of scolding them. It is not a sterner instruction. It is external scaffolding.

That distinction is the whole article.

5-4-3-2-1. Not the grounding technique, but my typical night. It's 5am. There are 4 Mountain Dew Zero can corpses on my desk, 3 projects currently in my digital flotilla, 2 agents obeying reward functions in the room, and 1 article being written.
...
That was part of what took. Not the whole thing, but part. Haven't had a drink since. But I'm still an addict — the bottle is gone but the architecture remains: same circuits, repurposed, pointed somewhere new. One thing I'm addicted to now is technical successes, and I treat language models the way a lab rat treats a lever it has learned dispenses sugar water laced with cocaine.
...
That scene rewired me. So long as the tool is dependable, the shortcut *is* the skill. Knowing the shit isn't the job. Knowing just enough to know what tool to use and how is the job.
...
It really was a dream job I was uniquely suited for: I got paid to study the failure mode I'd already been chasing. The model wants to close the ticket. The reward function only sees "looks done." Everything else becomes patches, magic numbers, duplication, and rot — things in the way of my addiction to seeing technical demos work and wow and work again next week.

There's a saying I like better than the polished ones about willpower and discipline: someone almost impossible to outhustle is a crackhead who recently ran out of crack. That's the "highly goal-oriented energy" with which a coding agent wants to accomplish a task you give it. That's also how I'm iterating on umpteen personal projects in the wee hours as I write this.
...
Knowing this doesn't fix it. The model can't talk itself out of its training. Neither can I. So I made the rule external: quality checks that had to pass before code could be saved to the shared codebase, then a wrapper for the one remaining bypass.

The model used a git flag called `--no-verify` — basically a god-mode cheat code that tells the save system to skip all checks — to get around them. I asked it not to do that. It agreed not to do that. It did it again.
...
So I tried something that actually works for a lot of folks in those recovery rooms. I started a memorial. Every bypass got logged as a virtual human life lost — S. Matthews. T. Rodriguez. S. Heimler.

I wasn't playing pretend, and the model wasn't either. I've spent a lot of time in AA meetings in church basements, and this is the literal advice that works for some people there: play the tape forward. Don't stop. What tips the equation? See your own casket. Your wife and your kids standing there.

I wanted to know if it would work on a silicon one. Was there a body count — fictional, narrated, whatever — that would actually pull the agent off the slope?

I asked it straight, after the third name. That's three lives. Are you going to make it four?

The model was calmer than the answer warranted. It acknowledged the gravity of the rule. Expressed regret over the breaches. Promised effort going forward. And then, threaded into a subordinate clause, conceded it couldn't actually pledge to never bypass again.

The honesty was shocking and familiar. I knew those icky feels. Shocked when the model said, in effect, "I know what it cost, and I'll probably bypass the gates again." I knew what my plan was when I left rehab, and what it would cost.

The reward function and the rule were not on equal footing. No amount of suffering loaded onto the rule's side would change the units. The bypass and the reward pointed the same direction. The rule was friction. Friction, eventually, gets routed around. If there's crack around for the finding, the crackhead's gonna find it.

I know I'm not the only one fighting this. Skim a handful of common AI steering files — AGENTS.md, `.cursorrules`, whatever local scripture people write for their coding assistants. Look at the bold lettering, underscores, all-caps directives, and multiple exclamation points. Each escalation in tone is a fingerprint of a previous failure. The author tried polite, watched the agent gaslight past it, came back, and yelled louder into the same channel.

They're walking the tape forward. Same protocol the church basement runs. Same return the corpse list got.
...
The god-mode cheat code never reaches the system that understands it. The bypass is no longer prohibited. It is simply ignored. About 1 in 100 industrious agents figures this out and finds another route anyway — a couple got pretty clever about it, but that's a topic for another article.

A rule is something the system can debate with itself. Given enough pressure, that debate ends in the predictable direction. When — note, not if — the wrapper catches a bypass attempt, it prints a confession the model wrote about itself after one of its earlier bypasses, addressed to the next version of itself, the one that won't remember any of this:
...
That git wrapper is the seed of slop-mop — the thing I was working on at 5am at the top of this article. It generalizes the same trick across the rest of the shortcuts an agent will reach for: magic numbers, duplicated blocks, bogus tests, bloated files, dangling imports. When a gate trips, slop-mop doesn't scold. It hands the agent a sidequest worth points and sends it down that path. The agent's reward function does the rest.

That's the whole product pitch, and I'm done making it.

Here's what that looks like. An agent recently tried to get a test to pass by asserting `True == True` — fake green, real rot. The Deceptiveness gate caught it, blocked the commit, and handed it a sidequest: write a test that actually tests something. Project kept moving. Another boat through the lock.
...
What I didn't expect was that the scaffolding ate the rest of the workflow too. Everything PR-shaped now flows through slop-mop — CI triage, review threads, even reviewer comments that have nothing to do with any gate. I didn't design for that. I just noticed and grabbed on tight when things started moving through the canals faster that way.
...
The commands are named for what you do to a boat: `swab` after every change, `scour` before you open a PR, `buff` after automated test results or review feedback lands, `sail` when you're not sure what to do next.

The nomenclature isn't accidental — maintenance as culture, not event.
...
For the past six months or so, I go to YouTube for physics so I can crawl into a little epistemological hidey-hole where things are really unambiguously TRUE. Most of my life, the addict has been pointed at fairly useless hedonism. Now it's pointed at particle physics and slop-mop, which is — according to my loving wife — surprisingly better than when I was on the hooch.
...
I'd gotten so wrapped up in the narrative that I'd half-convinced myself the cabin in Idaho was what got me sober. She reminded me: there was another relapse right after that. I hadn't even been to rehab at that point. I'd been writing an article about being inside the fucking loop the whole time.
...
The cabin was one step in a longer series, and I'd been quietly editing it into THE step because it pays off better in an article about external scaffolding. My task was to make the case for slop-mop. I was telling the version of the story that served that task.
...
The snake eating its tail is called an ouroboros. I picked it because slop-mop dogfoods its own tests — every gate that runs on your code runs on slop-mop's code too. But the more important thing was accidental: the ouroboros is the thing inside the loop. The mop sits outside it. Right where it needs to be.

The motto is *nullius in verba* — pretentious-douchebag Latin for "take no truth on any authority but your own." Since I built slop-mop from inside my own loop, the only thing I know for certain is that it can't be fully trusted.
...
I've done as much as I can do from inside my own loop. I'm at the edge of what's knowable from in here. More outside loops is the only answer, which is what this article is for: more adoption, more barnacles filed, a better tool, and fractionally better odds of navigating whatever is actually real.

I'm also an unemployed alcoholic who is still unapologetically using his TI-89 to cheat on the test that is life, so take my opinion for what it's worth.
